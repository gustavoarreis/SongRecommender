...
scope = [
    'user-read-playback-position','user-read-email', 'user-library-read', 
...
def get_token():
    """Get token from Spotify API if you don't already have one"""
...
def get_recs(reference_track):
    ...
    tracks = tracks.append(track, ignore_index = True)      # Join reference track with recommendations
...
def fetch_features(df):
    ...
    for i in df.index:
        features = sp.audio_features(tracks = df['id'][i])      # Get audio features for each track in df
...
def enhanced_rec(df, features, show = 25):
    """Sort recommendations selecting nearest neighbors. Select criteria of similarity(features)."""
    df["euclidian"] = float()
    a = []
    for n in features:
        a.append(df[n][100])
    for i in df.index:
        ...
        for n in features:
            b.append(df[n][i])
        df["euclidian"][i] = euclidian(point_a = a, point_b = b)        # Calculate euclidian distance
    df1 = df.sort_values("euclidian")[0:show]       # Sort values based on ascending euclidian distance
    return df1
...
def make_playlist(recs, song, features):
    ...
    description = str("Built using track's " + str(features) + " by Song Recommender")      # Set description
    playlist = sp.user_playlist_create(
        user = user, name = playlist_name, 
